...
def normalization_as_json( var_name, norm_dict, filepath=json_folderpath ) :
  ...
  # intro_var		= [ k for k,v in locals_function.items() ]
...
for categ, metatags in CATEGORIES_CIS_DICT.items() : 
...
NOMENCLATURE_CIS_LIST = [ {"code" : k , "fullname" : v["fullname"], "description" : v["description"] } for k,v in NOMENCLATURE_CIS_DICT.items() ]
...
for k,v in NORMALIZATION_TAGS_SOURCES_CIS_DICT.items() : 
  v.sort()
